...
def install_existing_packages(package_list, adb_command="adb", log_textbox=None):
    ...
    for package_name in package_list:
        message = f"Installing existing package: {package_name}"
        print(message)
        update_log(message)
...
def get_installed_packages(adb_command="adb", log_textbox=None):
    ...
    try:
        ...
        with open("installed_packages.txt", "w") as f:
            f.write("\n".join(packages))
        if log_textbox:
            log_textbox.insert(tk.END, "Installed packages list retrieved.\n")
            log_textbox.see(tk.END)
        return packages

    except subprocess.CalledProcessError as e:
        error_message = f"Error getting package list: {e.stderr}"
        messagebox.showerror("Error", error_message)
        if log_textbox:
            log_textbox.insert(tk.END, error_message + "\n")
            log_textbox.see(tk.END)
...
def create_gui():
    ...
    def handle_get_packages():
        ...
        packages = get_installed_packages(log_textbox=window.log_textbox)
        if packages:
    ...
    def save_selection():
        ...
        if selected_packages:
            ...
            try:
                ...
                window.log_textbox.insert(tk.END, "Selection saved.\n")
                window.log_textbox.see(tk.END)
            except Exception as e:
            ...
            try:
                ...
                for cb, selected, package in checkboxes:
                    if package in saved_packages:
                        selected.set(True)
                window.log_textbox.insert(tk.END, "Selection loaded.\n")
                window.log_textbox.see(tk.END)
            except Exception as e:
